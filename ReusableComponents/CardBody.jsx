...
export default function CardBody(props) {
  return (
    <div className="cursor-pointer w-full max-w-md mx-auto">
      <div className="relative group overflow-hidden rounded-2xl shadow-sm"> 
        ...
        {/* Like & View Icons */}
        <div className="absolute bottom-0 bg-gradient-to-t from-black/70 via-black/35 px-3 py-4 rounded-md z-50 w-full flex justify-around items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div>
            <p className="text-white text-md font-semibold text-center">{props.title}</p>
          </div>
      ...
      <div className="flex justify-between items-center h-14 sm:h-12 w-full mt-2">
        ...
        <div className="flex items-center gap-2">
          <Icon src="/assets/like1.jpeg" className="w-6 h-4" alt="Logo" />
          <p className="text-[12px]">{props.like}</p>
          <Icon src="/assets/view1.png" className="w-6 h-6" alt="Logo" />
          <p className="text-[12px]">{props.view}</p>
        </div>
      </div>
    </div>
  );
